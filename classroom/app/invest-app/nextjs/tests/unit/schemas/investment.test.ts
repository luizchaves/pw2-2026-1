...
describe('investmentSchema', () => {
  ...
  it('accepts investment without yield and with null dueDate', () => {
    ...
    expect(() => investmentSchema.parse({ ...rest, dueDate: null })).not.toThrow();
  });

  it('rejects non-UUID id', () => {
    expect(investmentSchema.safeParse({ ...validInvestment, id: 'not-a-uuid' }).success).toBe(
      false,
    );
  });

  it('rejects negative amount', () => {
    expect(investmentSchema.safeParse({ ...validInvestment, amount: -100 }).success).toBe(false);
  });

  it('rejects zero amount', () => {
    expect(investmentSchema.safeParse({ ...validInvestment, amount: 0 }).success).toBe(false);
  });

  it('rejects empty name', () => {
    expect(investmentSchema.safeParse({ ...validInvestment, name: '' }).success).toBe(false);
  });

  it('rejects invalid category', () => {
    expect(investmentSchema.safeParse({ ...validInvestment, category: 'Other' }).success).toBe(
      false,
    );
  });
});
...
describe('investmentFormSchema', () => {
  ...
  it('rejects empty name', () => {
    expect(investmentFormSchema.safeParse({ ...validForm, name: '' }).success).toBe(false);
  });

  it('rejects non-positive amountCents', () => {
    expect(investmentFormSchema.safeParse({ ...validForm, amountCents: 0 }).success).toBe(false);
  });

  it('rejects invalid yield format when provided', () => {
    expect(investmentFormSchema.safeParse({ ...validForm, yield: 'invalido' }).success).toBe(false);
  });

  it('accepts empty string yield (treated as absent)', () => {
    expect(investmentFormSchema.safeParse({ ...validForm, yield: '' }).success).toBe(true);
  });
});
