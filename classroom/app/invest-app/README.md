...
- Tailwind CSS
- Storage local e modulos simples em `src/`
...
Nao oferece uma arquitetura robusta por padrao para estado, rotas, formularios complexos, testes de componentes ou integracao autenticada. Escalar a aplicacao exige criar convencoes manualmente.
...
- **Modulos ES**: separacao simples entre dados, storage e entrada da aplicacao.
- **Local storage**: persistencia local sem backend.
...
| Estado | Zustand + TanStack Query | Signals + services Angular | Pinia + TanStack Vue Query | Modulos JS + local storage |
| Formularios | React Hook Form | Reactive Forms | Estado reativo + Zod manual | DOM events e validacao manual |
| Validacao | Zod + resolver do React Hook Form | Validators do Angular + validators customizados | Zod com `safeParse` | Regras manuais no JavaScript |
| Dados | Fetch para API Routes Next | Supabase client direto nos services | Supabase client direto nos services | Local storage |
| Auth | Supabase Auth no client + token nas API Routes | Supabase Auth no service Angular | Supabase Auth no store Pinia | Nao possui auth real |
| UI | shadcn/base-ui style + Tailwind CSS | Componentes standalone + CSS global proprio | Componentes Vue + Tailwind CSS | HTML, CSS e DOM API |
| Icones | `lucide-react` | `@lucide/angular` | `lucide-vue-next` | Sem biblioteca de icones |
| Testes unit/component | Vitest + Testing Library React | Angular unit test builder + Vitest + TestBed | Nao configurado nesta versao | Nao configurado nesta versao |
| E2E | Playwright | Playwright | Nao configurado nesta versao | Nao configurado nesta versao |
...
No Angular e no Vue.js, o app ficou como **SPA pura**: nao ha backend embutido. Por isso, a logica das API Routes foi adaptada para services/stores que chamam o Supabase diretamente pelo client.

No Vite Vanilla, o app e uma **SPA local e didatica**: nao ha autenticacao nem persistencia remota; o foco e manipular DOM, eventos, modulos e storage local.
