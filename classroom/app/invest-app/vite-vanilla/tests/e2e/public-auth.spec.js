...
test('landing page shows public navigation and CTAs', async ({ page }) => {
  ...
  await expect(
    page.getByRole('heading', { name: 'Acompanhe sua carteira sem planilhas soltas.' }),
  ).toBeVisible()
  await expect(page.getByRole('navigation').getByRole('link', { name: 'Entrar' })).toHaveAttribute(
    'href',
    '/login',
  )
  await expect(
    page.getByRole('navigation').getByRole('link', { name: 'Cadastrar' }),
...
test('register validates password confirmation before submitting', async ({ page }) => {
  ...
  await page.getByPlaceholder('Mínimo de 6 caracteres').fill('segredo123')
